  ...
  try {
    ...
    const [rows] = await db.execute(
      ...
      SELECT
        ...
        COALESCE(
          sel.line_type,
          CASE WHEN latest.response_line_id IS NOT NULL THEN 'DEMAND' ELSE NULL END
        ) AS selected_line_type,
        COALESCE(
          sel.line_type,
          CASE WHEN latest.response_line_id IS NOT NULL THEN 'DEMAND' ELSE NULL END
        ) AS selected_selection_types,
        COALESCE(sel.line_label, reqop.part_number, cri.client_part_number) AS selected_line_label,
        COALESCE(sel.line_label, reqop.part_number, cri.client_part_number) AS selected_selection_labels,
        COALESCE(sel.line_description, cri.client_description, reqop.description_ru, reqop.description_en) AS selected_line_description,
        COALESCE(sel.line_description, cri.client_description, reqop.description_ru, reqop.description_en) AS selected_selection_descriptions,
        COALESCE(sel.oem_part_id, latest.oem_part_id, cri.oem_part_id) AS selected_original_part_id,
        sel.alt_oem_part_id AS selected_alt_original_part_id,
        COALESCE(sel.standard_part_id, latest.standard_part_id, cri.standard_part_id) AS selected_standard_part_id,
...
router.post('/manual-line', async (req, res) => {
  ...
  try {
    ...
    const incotermsPlace = normalizeIncotermsPlace(req.body.incoterms_place)
    const originCountry = normalizeCountryCode(req.body.origin_country)
    const note = nz(req.body.note)
  ...
  try {
    ...
    const next = {
      ...
      incotermsPlace: hasOwn(req.body, 'incoterms_place')
        ? normalizeIncotermsPlace(req.body.incoterms_place)
        : baseLine.incoterms_place,
      originCountry: hasOwn(req.body, 'origin_country')
        ? normalizeCountryCode(req.body.origin_country)
        : baseLine.origin_country,
    ...
    await writeLineAction(conn, {
      ...
      payload: {
        ...
        next_origin_country: next.originCountry,
      },
      createdByUserId,
    })
